...
class PresupuestoProvider with ChangeNotifier {
  ...
  Future<void> fetchPresupuestos() async {
    ...
    try {
      ...
      debugPrint("Error en provider de presupuestos: $error");
    }
    _isLoading = false;
    notifyListeners();
  }
  ...
  Future<void> updatePresupuesto(Presupuesto presupuesto) async {
    try {
      ...
      debugPrint("Error al actualizar presupuesto: $error");
      rethrow; // Lanza el error para que la UI pueda mostrar un mensaje
    }
  }
  ...
  Future<void> deletePresupuesto(int id) async {
    try {
      ...
      debugPrint("Error al eliminar presupuesto: $error");
      rethrow; // Lanza el error para que la UI pueda mostrar un mensaje
    }
  }
}